...
const checkExisting = async () => {
    ...
    try {
        const res = await fetch(route('produksi.check-existing', {
...
watch(() => [form.blok_id, form.tanggal], () => {
    checkExisting();
}, { immediate: true });
...
const totalJanjang = computed(() => {
    return form.items.reduce((sum, it) => sum + (Number(it.jumlah_janjang) || 0), 0);
});
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-emerald-700 dark:text-emerald-400">
                        Input rincian pemanen, janjang, dan timbangan per blok afdeling
                    </p>
                </div>
                <Link
        ...
        <div class="py-8">
            <div class="mx-auto max-w-5xl sm:px-6 lg:px-8 space-y-6">
                ...
                <div
                    ...
                    class="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800 dark:border-rose-900/50 dark:bg-rose-950/30 dark:text-rose-300 flex items-start gap-3"
                >
                    ...
                    <div>
                        <span class="font-bold">Perhatian:</span> Data produksi blok dan tanggal ini sudah tervalidasi dan disetujui oleh Asisten Afdeling. Anda tidak diizinkan untuk mengubahnya kembali.
                    </div>
                </div>

                <div
                    v-else-if="existingStatus === 'menunggu'"
                    class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-300 flex items-start gap-3"
                >
                    <svg class="h-5 w-5 shrink-0 text-amber-600 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <div>
                        <span class="font-bold">Mode Edit:</span> Rekaman data panen blok dan tanggal ini sudah tercatat (menunggu validasi asisten). Mengirimkan form ini akan memperbarui rincian data yang sudah ada.
                    </div>
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
                                    Blok Panen <span class="text-rose-500">*</span>
                                </label>
                                <select
                                ...
                                >
                                    <option value="" disabled>Pilih Blok di Afdeling</option>
                                    <option v-for="b in bloks" :key="b.id" :value="b.id">
                                        Blok {{ b.kode_blok }} ({{ b.luas_ha }} Ha • {{ b.jumlah_pokok }} Pokok)
                                    </option>
                                </select>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
                                    Tanggal Panen <span class="text-rose-500">*</span>
                                </label>
                                <input
                                    ...
                                    :disabled="existingStatus === 'disetujui'"
                                />
                                <p v-if="form.errors.tanggal" class="text-xs text-rose-600 mt-1">{{ form.errors.tanggal }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border border-emerald-900/10 bg-white p-6 shadow-sm dark:border-emerald-500/10 dark:bg-gray-800 space-y-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    Pilih pekerja panen aktif dan masukkan jumlah janjang serta berat timbangan masing-masing
                                </p>
                            </div>
                            <button
                                v-if="existingStatus !== 'disetujui'"
                        ...
                        <div class="space-y-3">
                            <div
                                ...
                                :key="idx"
                                class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-3 rounded-lg border border-gray-100 bg-gray-50/50 dark:border-gray-700 dark:bg-gray-900/50"
                            >
                                ...
                                <div class="flex-1">
                                    <select
                                        ...
                                        :disabled="existingStatus === 'disetujui'"
                                    >
                                        <option value="" disabled>Pilih Pemanen</option>
                                        <option v-for="p in pemanens" :key="p.id" :value="p.id">
                                            {{ p.kode_pemanen }} - {{ p.nama }}
                                        </option>
                                    </select>
                                    <p v-if="(form.errors as any)[`items.${idx}.pemanen_id`]" class="text-xs text-rose-600 mt-1">
                                        {{ (form.errors as any)[`items.${idx}.pemanen_id`] }}
                                    </p>
                                </div>
                                ...
                                <div class="w-full sm:w-36">
                                    <input
                                        ...
                                        min="1"
                                        placeholder="Janjang"
                                        class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 text-xs focus:border-emerald-500 focus:ring-emerald-500 text-right font-mono"
                                        required
                                        :disabled="existingStatus === 'disetujui'"
                                    />
                                    <p v-if="(form.errors as any)[`items.${idx}.jumlah_janjang`]" class="text-xs text-rose-600 mt-1">
                                        {{ (form.errors as any)[`items.${idx}.jumlah_janjang`] }}
                                    </p>
                                </div>
                                ...
                                <div class="w-full sm:w-36">
                                    <input
                                        ...
                                        :disabled="existingStatus === 'disetujui'"
                                    />
                                    <p v-if="(form.errors as any)[`items.${idx}.berat_kg`]" class="text-xs text-rose-600 mt-1">
                                        {{ (form.errors as any)[`items.${idx}.berat_kg`] }}
                                    </p>
                                </div>

                                <div class="flex items-center justify-end">
                                    <button
                                        v-if="form.items.length > 1 && existingStatus !== 'disetujui'"
                                        type="button"
                                        @click="removeRow(idx)"
                        ...
                        <div class="mt-6 rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 dark:border-emerald-800 dark:bg-emerald-950/20">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center sm:text-left">
                                <div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Pemanen Bekerja</div>
                                    <div class="text-lg font-bold text-gray-900 dark:text-white font-mono">
                                        {{ selectedPemanenCount }} Orang
                                    </div>
                                </div>
                                <div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Total Janjang</div>
                                    <div class="text-lg font-bold text-gray-900 dark:text-white font-mono">
                                        {{ totalJanjang.toLocaleString('id-ID') }} Jjg
                                    </div>
                                </div>
                                <div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Total Berat</div>
                                    <div class="text-lg font-bold text-emerald-700 dark:text-emerald-400 font-mono">
                                        {{ totalBeratKg.toLocaleString('id-ID', { minimumFractionDigits: 1 }) }} Kg
                                    </div>
                                </div>
                                <div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Rata-rata BJR</div>
                                    <div class="text-lg font-bold text-gray-900 dark:text-white font-mono">
                                        {{ averageBjr }} Kg/Jjg
                                    </div>
                                </div>
                    ...
                    <!-- Notes Card -->
                    <div class="rounded-xl border border-emerald-900/10 bg-white p-6 shadow-sm dark:border-emerald-500/10 dark:bg-gray-800 space-y-2">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        ...
                        <button
                            type="submit"
                            :disabled="form.processing || existingStatus === 'disetujui'"
                        ...
                        >
                            {{ form.processing ? 'Menyimpan...' : (existingStatus === 'menunggu' ? 'Perbarui Data Panen' : 'Simpan Produksi Panen') }}
                        </button>
                    </div>
                </form>
            </div>
