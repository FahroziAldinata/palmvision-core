...
const approve = (item: ProduksiItem) => {
    if (
        confirm(
            `Setujui rekaman panen Blok ${item.blok?.kode_blok} (${item.tanggal})?`,
        )
    ) {
        approvingId.value = item.id;
        router.post(
            route('produksi.validasi.approve', item.id),
            {},
            {
                onFinish: () => {
...
const submitKoreksi = () => {
    ...
    koreksiForm.post(
        route('produksi.validasi.koreksi', correctingItem.value.id),
        {
            onSuccess: () => {
...
<template>
    ...
    <AuthenticatedLayout>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-emerald-700 dark:text-emerald-400">
                        Pemeriksaan berjenjang hasil panen mandor oleh Asisten
                        Afdeling (Audit Trail aktif)
                    </p>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                ...
                <div class="flex border-b border-gray-200 dark:border-gray-700">
                    <Link
                        :href="
                            route('produksi.validasi.index', {
                                tab: 'menunggu',
                            })
                        "
                        class="flex items-center gap-2 border-b-2 px-5 py-3 text-xs font-semibold transition-colors"
                        :class="
                            currentTab === 'menunggu'
                    ...
                    >
                        ...
                        <span
                            v-if="
                                currentTab === 'menunggu' && produksis.total > 0
                            "
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    <td
                                        class="px-6 py-4 text-right font-mono font-semibold text-gray-900 dark:text-gray-100"
                                    >
                                        {{
                                            Number(
                                                item.total_janjang,
                                    ...
                                    <td
                                        class="px-6 py-4 text-right font-mono text-xs text-gray-600 dark:text-gray-300"
                                    >
                                        {{
                                            item.total_janjang > 0
                                                ? (
                                                      item.total_berat_kg /
                                                      item.total_janjang
                                                  ).toFixed(2)
                ...
                >
                    ...
                    <div
                        class="max-h-72 divide-y divide-gray-100 overflow-y-auto dark:divide-gray-700/60"
                    >
                        <div
                            v-for="log in auditLogs"
                            :key="log.id"
                            class="space-y-1 py-3 text-xs"
                        >
                            <div
                            ...
                            <div
                                v-if="log.properties?.alasan"
                                class="rounded bg-amber-50 p-2 text-amber-700 dark:bg-amber-950/30 dark:text-amber-400"
                            >
                                <span class="font-semibold"
                                    >Alasan Koreksi:</span
                                >
                                {{ log.properties.alasan }}
                            </div>
                            <div
                                v-if="
                            ...
                            >
                                Janjang:
                                {{ log.properties.old.total_janjang }} ➔
                                {{ log.properties.new.total_janjang }} | Berat:
                                {{ log.properties.old.total_berat_kg }} Kg ➔
                                {{ log.properties.new.total_berat_kg }} Kg
                            </div>
                        </div>
                        <div
                        ...
                        >
                            Belum ada riwayat audit trail validasi yang
                            tercatat.
                        </div>
                    </div>
                </div>
        ...
        >
            <div
                class="w-full max-w-xl rounded-2xl border border-gray-100 bg-white p-6 shadow-xl dark:border-gray-700 dark:bg-gray-800"
            >
                <h3
                    class="mb-2 text-base font-bold text-gray-900 dark:text-white"
                >
                    Koreksi Data Panen Blok {{ correctingItem.blok?.kode_blok }}
                </h3>
                <p class="mb-4 text-xs text-gray-500 dark:text-gray-400">
                    Setiap perubahan angka akan dicatat ke dalam audit trail
                    beserta alasan koreksi Anda.
                </p>

                <form @submit.prevent="submitKoreksi" class="space-y-4">
                    <div class="max-h-60 space-y-2 overflow-y-auto pr-1">
                        <div
                            v-for="row in koreksiForm.items"
                            :key="row.id"
                            class="flex items-center gap-2 rounded-lg bg-gray-50 p-2 text-xs dark:bg-gray-900/50"
                        >
                            <div
                                class="flex-1 font-medium text-gray-800 dark:text-gray-200"
                            >
                            ...
                            <div class="w-28">
                                ...
                                <input
                                    ...
                                    type="number"
                                    min="1"
                                    class="w-full rounded border-gray-300 text-right font-mono text-xs dark:border-gray-700 dark:bg-gray-900"
                                    required
                                />
                            </div>
                            <div class="w-28">
                    ...
                    <div>
                        ...
                        >
                            Alasan Koreksi
                            <span class="text-rose-500">* (Audit Trail)</span>
                        </label>
                        <textarea
                        ...
                        ></textarea>
                        <p
                            v-if="koreksiForm.errors.alasan_koreksi"
