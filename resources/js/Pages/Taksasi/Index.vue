            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-emerald-700 dark:text-emerald-400">
                        Daftar estimasi panen historis per blok afdeling (tersimpan permanen)
                    </p>
                </div>
                        ...
                        <table class="w-full text-left text-sm text-gray-600 dark:text-gray-300">
                            <thead class="border-b border-gray-100 bg-emerald-50/50 text-xs font-semibold uppercase text-emerald-900 dark:border-gray-700 dark:bg-gray-900/50 dark:text-emerald-300">
                                <tr>
                                    ...
                                    <th class="px-6 py-3.5 text-right">Sampel (Pkk/Jjg)</th>
                                    <th class="px-6 py-3.5 text-right">AKP</th>
                                    <th class="px-6 py-3.5 text-right">BJR (Kg)</th>
                                    <th class="px-6 py-3.5 text-right">Estimasi Total (Kg)</th>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
                                ...
                                >
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            {{ item.blok?.afdeling?.nama || '-' }} ({{ item.blok?.jumlah_pokok || 0 }} Pkk)
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-right font-mono text-xs">
                                        {{ item.pokok_disampel }} / {{ item.estimasi_janjang }}
                                    </td>
                                    <td class="px-6 py-4 text-right font-mono font-semibold text-emerald-800 dark:text-emerald-400">
                                        {{ item.pokok_disampel > 0 ? (item.estimasi_janjang / item.pokok_disampel).toFixed(2) : '0.00' }}
                                    </td>
                                    <td class="px-6 py-4 text-right font-mono text-xs">
                                        {{ Number(item.estimasi_bjr).toFixed(1) }}
                                    </td>
                                    <td class="px-6 py-4 text-right font-mono font-bold text-gray-900 dark:text-gray-100">
                                        {{ Number(item.estimasi_total_kg).toLocaleString('id-ID') }}
                                    </td>
                                ...
                                <tr v-if="taksasis.data.length === 0">
                                    <td colspan="7" class="px-6 py-12 text-center text-sm text-gray-500">
                                        Belum ada riwayat taksasi panen yang dicatat.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
